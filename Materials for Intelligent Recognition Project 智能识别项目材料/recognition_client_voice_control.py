...
# 启动图片预览线程
camera_thread=threading.Thread(target=image_preview, args=(0,))
camera_thread.start()
...
def recognize(url):
    ...
    logging.debug("Thread is launched with URL {}".format(url))
    ...
    while running:
        ...
        if data == b'id_on\n':
            ...
            if response.status_code == 200:
                ...
                hex_string = "AA550155AA"
                byte_string = binascii.unhexlify(hex_string)
                SerialObj.write(byte_string)
...
recognize_thread=threading.Thread(target=recognize, args=("http://127.0.0.1:5000",))
